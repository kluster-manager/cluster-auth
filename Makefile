...
install:
	...
	helm upgrade -i cluster-auth-manager charts/cluster-auth-manager --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall cluster-auth-manager --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/cluster-auth run \
		--v=3 \
		--secure-port=8443 \
